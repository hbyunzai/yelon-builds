...
/**
 * Fix https://github.com/hbyunzai/ng-yunzai/issues/2359
 */
...
function addRunScriptToPackageJson(): Rule {
  return (tree: Tree) => {
    ...
    json.scripts[`${commandPrefix}icon`] = `ng g ng-yunzai:plugin icon${
...
function addSchematics(options: ApplicationOptions): Rule {
  return updateWorkspace(async workspace => {
    ...
    schematics['ng-yunzai:edit'] = {
      skipTests: false,
      modal: true
    };
    schematics['ng-yunzai:view'] = {
      skipTests: false,
      modal: true
    };
    schematics['ng-yunzai:curd'] = {
      skipTests: false
    };
...
export default function (options: ApplicationOptions): Rule {
  return async (tree: Tree, context: SchematicContext) => {
    ...
    context.logger.info(`Generating NG-YUNZAI scaffold to ${projectName} project...`);
    return chain([
      // @yelon/* dependencies
