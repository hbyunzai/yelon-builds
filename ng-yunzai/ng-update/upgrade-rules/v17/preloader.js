...
function run(tree, name, sourceRoot, context) {
    ...
    [`import { preloaderFinished } from '@yelon/theme';`, 'preloaderFinished();'].forEach(item => {
    ...
    const importIndex = appContentLines.findIndex(line => line.includes(', VERSION as VERSION_YUNZAI'));
    ...
    if (importIndex === -1 || addIndex === -1 || callDoneIndex === -1)
        return;
    appContentLines[importIndex] = appContentLines[importIndex].replace(', VERSION as VERSION_YUNZAI', ', VERSION as VERSION_YUNZAI, stepPreloader');
