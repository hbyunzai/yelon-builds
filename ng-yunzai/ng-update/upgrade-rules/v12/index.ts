...
function finished(): Rule {
  return (_tree: Tree, context: SchematicContext) => {
    context.logger.warn(
      colors.yellow(
        `  ✓  After the upgrade is complete, you still need to execute \`ng lint --fix\` to fix the code format, Abort more detail please refer to upgrade guide https://ng.yunzainfo.com/docs/style-guide`
      )
    );
    context.logger.info(
      colors.green(`  ✓  Congratulations, Abort more detail please refer to upgrade guide https://github.com/ng-yunzai`)
    );
  };
}
...
export function v12Rule(options: Schema): Rule {
  return async (tree: Tree, context: SchematicContext) => {
    logStart(context, `Upgrade @yelon/* version number ${JSON.stringify(options)}`);
